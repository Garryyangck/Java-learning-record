	...
	- [剑指 Offer 43. 1～n 整数中 1 出现的次数](https://leetcode.cn/problems/1nzheng-shu-zhong-1chu-xian-de-ci-shu-lcof/)
		...
		public int digitOneInNumber(int n) {
		    ...
		    while (bit <= n) { // 思路，bit增大后，n / bit，这样不断缩小n进行归约
		        ...
		        // 比如 501222，当bit=100时，此时计算的是百位，cur指向第一个2
		        // high = (n/bit)/10，在这里为501
		        // cur = (n/bit)%10，在这里为2
		        // low = n%bit，在这里为22
		        long high = n / bit / 10;
		        long cur = (n / bit) % 10;
		        ...
		        if (cur > 1) {
		            // 此时如果cur>1，说明百位上可以为1
		            // 那么百位上到底有多少个1呢，就用high的个数乘low的个数即可
		            // 比如此时cur=2，说明百位包含了1
		            // 那么百位令为1，排列组合high和low就是百位为1时的总数
		            // 只需按照此思路把百位为1的个数，千位为1的个数加起来即可
		            sum += (high + 1) * bit;
		        } else if (cur == 1) {
		            // 当前位是1，则在 501222 中，左边有0~500，右边有bit种可能，
		            // 并且当左边是501(最大时)，右边可以为0~low，共计low+1个
		            sum += (high * bit) + (low + 1)/*后面的代表low部分数的个数，即low+1*/;
		        } else {
		            // 如果当前位是0，比如 501222 中的0，那么在万位上还有 491xxx 类似的含1的情况
		            // 在 501222，中左边只能取0~4，右边取bit个
		            sum += high * bit;
		        }
		        bit = b it * 10;
		    }
...
1. 今天完善自己在各个招聘网站上的信息，并开始在各个网站上投了几个。
2. 加油吧，一边投递一边查漏补缺的生活。
...
1. 今天复习了排序算法，包括不太熟悉的插入和希尔排序、堆排序以及快忘了的快速排序。
2. 今天投递的简历都石沉大海，也算是意料之中，毕竟目前只从第三方平台投递，而且投递的数目也不多。不过比起投递的石沉大海，更让我感到焦虑的是我对于各项八股文以及项目基本上算是全忘了，从这种意义上说，没人理我其实算是一件“好事”，让我可以立刻发觉自己的不足，并作出调整。
3. 最近又感到了久违的焦虑感，之前是对前路无从选择的迷茫感，而现在则是意识到自身能力不足的危机感。上次的焦虑情绪来自在寒假，然后我发现了地哥，了却了那种焦虑；而这次同样是假期开始，熟悉的焦虑感涌上心头，但愿这次也能像上次那样寻找到某个“机遇”，获得暂时的解脱。



## 2024.6.27打卡 Day 148

1. 今天感到有点崩溃，主要是受到了周围人的影响，产生一种周围人都有方向就自己还没有方向的感觉。
2. 今天主要在胡思乱想，顺便做一下堆积如山的意义不明的大作业。
3. 我居然产生了放弃的想法，但这才刚开始啊。
4. 这是正常现象吗，是我想多了吗
...
2. Be brave and embrace life. 
3. 离开了令自己身心俱疲的学校，重新审视现状后，是时候开启一段新的旅途了。



## 2024.7.1打卡 Day 150

1. 今天开始第二阶段的学习，首先是阅读《自顶向下方法》第一章，但是感觉读起来比较吃力，因此就找了一门公开课先看，看完了之后再回过头看书会轻松不少。
2. 复习Java相关的八股文，复习速度还不错，就是不知道有没有全部记住了。



## 2024.7.7打卡 Day 151

1. 在菜鸟教程看了 python 基础语法和 FastAPI 入门。
2. 计算机网络课程看了1.2网络边缘
...
2024 年 8 月 6 日，父母劝我读研，他们说读研了可以进国企，以后不会被裁。

2024 年 8 月 16 日，在临别前，leader 建议我读研，他认为我不应该拘泥于 Java，希望我能去了解，去见识，去体验更多的方向，更多的技术，找到自己热爱的技术，能投入一生的方向。

2024 年 8 月、9 月，铺天盖地的考研宣传让我抵触，我在考研和找工作中挣扎。那种熟悉的迷茫感又来了，这导致我两个月，几乎没有什么进展。

2024 年 10 月 21 日，我在不堪入目的生活中回想起上学期满腔热血的自己，那时候每一天都是坚持向前、不甘落后。我想我不应该在分岔路口处停滞不前，而应该在挣扎中前行。



## 2024.10.21打卡 Day 1

1. CMU-csapp: Machine Level Programming II: Controll

	不得不说作者讲得就是比学校老师好得多，学校老师半天扯不清楚的东西，作者两句话就说完了，给我一种学校老师没备课的感觉。csapp 能帮你把一些很常用，但是你很少了解的东西给你打通；以及一些很常见，但是你几乎不会去解决的问题讲清楚。虽然学校开了这门课，但真要学到东西还是得原版的课+教材啊。

2. 成功在 Spring boot 3 + Spring Cloud 2022.0.0 + Alibaba Spring Cloud 2022.0.0.0-RC1 的环境下集成 Nacos 2.2.1 注册中心，并实现在 Nacos 上进行多环境配置和动态修改配置。



## 2024.10.22打卡 Day 2

1. 汇编语言补上了第二章，这章概念比较多，所以花的时间比我想象中多一些。但感觉汇编语言这些东西学了也对我帮助不大，内容基本上就是给你一大堆底层东西的概念特性让你背，这门课的质量感觉远远不如 csapp。

2. CMU-csapp: Machine Level Programming III: Produsures

	通过栈模拟过程切换中的“先进后出”逻辑，这回答了为什么使用栈而不是其它数据结构。同时解释了栈的运行原理，以及栈帧包含的元素，这些元素的作用等等。最后 caller-saved registers 和 callee-saved registers 分别存储函数调用（过程切换）前后，不需要和需要保持不变的局部变量，以及它们在非递归和递归调用中的作用的演示。

3. Nacos 注册中心的集成，以及配置 GatewayNacos 和 FeignNacos 实现负载均衡，不需要我写死路由了。最后知道所有应用连接 Nacos 之后，Nacos 会生成一个路由表，推送给所有的应用。所以 Nacos 就算短暂的挂了，所有的服务也能正常启动。因此 Nacos 短暂的挂了之后，控制台寄了，但是服务还是可以正常运行。（为什么说短暂的，因为如果在 Nacos 挂了的这段时间里，新的服务加入进来，那么这时，之前的哪些服务就无法识别到这个新加的服务，gateway 也无法转发这个服务的请求。）



## 2024.10.23打卡 Day 3

1. 学习 spring boot 中缓存的种类、使用方式，以及它们的优缺点、产生的问题以及的解决的方式。包括 mybatis 一级、二级缓存，spring boot 内置缓存，spring boot cache 整合 redis 的缓存。对于 redis 缓存，详细了解其缓存击穿、缓存穿透和缓存雪崩从产生原因，以及多种解决方法，并通过 demo 加深理解。最后学到前端 vue3 下的会话和本地缓存，并使用本地缓存保存用户登录信息，使得用户关闭浏览器再次登录时，能从 localStorage 中读取登录信息直接完成登录。



## 2024.10.24打卡 Day 4

1. CMU-csapp: Machine Level Programming IV: Data

  从是否能通过编译，是否会引起空指针异常，在内存中占用的字节大小三个方面区分数组和指针，并强化了对指针的阅读方法（从内向外）。区分多维数组和嵌套数组，前者在内存空间中连续，后者本质是一个元素为数组指针的指针数组，因此数组指针指向的数组们，在内存中并不连续。为避免从内存读取数据页时 struct 被分割到两页，需要对 struct 进行对齐处理，并通过将对齐需求大的数据类型放到前面（不绝对）达到节省 struct 空间的效果。

  > 本想着学完了做一下 bomblab 的，结果学校的实验平台出问题了，仓库是空的，真是无语了…

2. 学习 Seata 的基本思想和四种模式。

2. 理清了 localStorage 和 cookie 在存储位置、与服务器交互的方式、安全性、跨域间的区别。
...
1.  csapp-lab2-bomblab
	...
	今天做了一整天，把 phase_1 ~ phase_6 做完了，还剩一个 secret_phase 实在干不动了，明天再说。虽然看汇编代码很费眼睛，但是把 lab 解出来的时候还是很有成就感的。当然重要的一方面是可以显著提高你的内存的理解，包括什么指针、栈区、栈指针、全局变量区、字符串常量区，以及递归的原理、结构体的原理什么的全都不在话下。
...
1. CMU-csapp: Machine Level Programming V: Advanced Topics

	主要是两部分，第一部分是内存分配，这里比较简单，值得一提的是 x86_64 虽然是 64 位，但是内存地址中只使用了 47 位，并且 stack 的栈底位于最高地址处，Text 区位于最低地址处，Heap 的大对象放到高地址，小对象放到低地址，然后两边指针向中间靠。

	另一部分则讲到利用栈溢出的漏洞，覆盖栈区的 return address 导致栈溢出出错，或者在栈区注入攻击代码然后将 return address 覆盖为栈区上注入的攻击代码的首地址，实现任意逻辑的攻击。x86_64 系统针对 DI 攻击的对策是在栈底之上添加随机长度的空地址，避免黑客推测到栈的准确地址；将栈区权限设置为不可执行防止执行注入代码，并使用 Canary，在栈帧的缓冲区和状态信息之间存储一个 Canary 值检测可能出现的栈溢出危险。针对前两个防护，可以采用 ROP 思想，在存储源代码 Text 区中截取多个 Gadgets，将 Gadgets 的首地址注入到 return address，绕开栈区不可执行的防护实现攻击。

	已经迫不及待想去做 AttackLab 了。

2. 操作系统进程管理和并发执行，这部分由于之前浅学过相应的八股文，所以自学起来没什么难度。不过也算了解了进程的 PCB，原语，和常见的调度算法。后面的部分和之前的八股很类似，比如线程vs进程，通过信号量实现进程并发执行过程中对临界资源的访问的隔离，一些常见的进程并发执行问题，比如 PC 问题，理发师问题等，还有死锁的形成条件、预防方法和检测方法，以及银行家算法。

	总之，操作系统这部分可以当作是对操作系统八股文的复习，和对之前不够清楚的问题的一次深刻思考。



## 2024.10.30打卡 Day 8

1. 分析并解决高并发场景下超卖的问题。JDK 锁的生效范围是单个服务节点，在分布式的环境下无法保障并发安全。利用 setnx 和 delete 命令实现的 redis 分布式锁的缺点是会设置分布式锁的过期时间，如果在锁过期前没能执行完毕，其它线程就能拿到锁，引发并发危险。解决该问题的方法之一是使用看门狗(Watch Dog)机制，创建守护线程不断重置锁过期时间，直到主线程执行完毕或出现异常终止。但是上述看门狗机制的缺点是，只能适用于单节点 redis 的情况，因此一旦 redis 节点宕机，分布式锁就无法运行。因此可以使用 Redis RedLock，配置奇数个 redis 节点，只有拿到半数以上的分布式锁，才能算成功，这样既不会出现并发风险，也不会因为低于半数的节点宕机而使服务终止。RedLock 的缺点是如果获取多个节点的时间大于过期时间，就会释放开始时获取的分布式锁，因此需要给获取锁操作设置超时时间。另外节点宕机，立刻上从节点或重启节点，都会出现之前的分布式锁没有释放，但是该节点又出现了新分布式锁的问题，因此上从节点或重启节点必须等待之前的分布式锁释放后，才能进行。

2. CMU-csapp: Bits、Byte and Integer I

	位运算，注意 &&、||、! 的结果为 0x0 或 0x1。无符号数和有符号数的转换，即负数 + $2^{w}$。C 语言在涉及 signed 和 unsigned 的运算时，会将有符号数隐式转换为无符号数。符号扩展，截断无论对于有符号还是无符号都是直接截去高 K 位。

3. 了解限流、熔断、服务降级的基础概念，触发时机、恢复时机。了解 Sentinel 组件的限流降级和熔断降级功能。
...
2. CMU-csapp: Bits、Byte and Integer II

	有符号数和无符号数的加法，都是采取低位直接截断；它们的乘法也采取低位直接截断，并且可以将乘任何数改写为移位和加法的混合运算；除法除以 2 的幂次时，负数需要加上 $2^k-1$ 偏移量，保证结果向 0 取整；-0x80000000 = 0x80000000，这是补码数不对称性，导致的一个特例。
