...
1. ==网络==：
	...
	> ==节点和边==构成，比如蜘蛛网也是一种网络，神经元也是一种网络。

2. ==计算机网络==：本质也是网络，因此有节点和边缘。

	> - ==节点，分为主机节点和数据交换节点==。主机节点比如一个 web 服务器；数据交换节点比如路由器、交换机，它既不是源，也不是目标，而是起到转发数据的作用。
	> - ==边，分为通信链路，分为接入网链路和主干链路==。接入网链路是主机连接到互联网的链路，而主干链路则是路由器之间的链路。
	
3. ==互联网==：

	> 由 ==tcp/ip 协议为主的一簇协议构成的网络==。互联网是由==一堆网络，通过网络互联设备（如路由器）连接在一起==的，而网络里面也会包含小网络，这些小网络之间也可以进行连接。

4. ==协议==：

	> 协议定义了在==两个或多个通信实体之间交换的报文格式和次序（每一层的 PDU 不一样）==，以及在报文传输和接收或其他事件方面所采取的动作。（可以理解为一种同一层次不同实体之间交流的规范）

5. ==互联网==从==构成的角度==来看，可以分为==网络边缘、网络核心、接入系统==。
...
1. ==端系统==：
	...
	> 设备上能进行通讯的==网络操作系统==，还有在==操作系统上面驻留的应用程序（进程，比如游戏，邮件，其它服务系统等）==。

2. ==客户端/服务端模式==（Client/Server）

	> 缺点：==可扩展性不强==，因为如果要增加服务器部署的数量的话，那么必然会==增加部署大量的物理连接链路==，并且当客户端请求数超过阈值的时候，性能断崖式下降。

3. ==P2P 模式==：

	> 一个客户端可以向多个服务端请求资源，这样就避免了 CS 模式下服务器扩容速度跟不上用户请求增加速度的问题。

4. ==面向连接的通信==：

	> 两个端系统（应用进程）之间先建立连接，比如握手、交换资源后，再进行通信。
...
1. ==线路交换==：端到端之间分配==独享的线路==，==不会出现资源（比如带宽）共享的问题==。

      > - ==优点：性能保障==，两个端系统间通过电路交换建立的线路是由这两个端系统独享的。比如==电话通信==。
...
3. ==POP==: 
	...
	> ==高层== ISP 面向==客户网络==的接入点，涉及费用结算。比如一个低层 ISP 接入多个高层 ISP，多宿。
...
1. ==延时的种类==：
	...
	> - ==传输延时==。（带宽 / 分组长度）
	> - ==传播延时==。（介质长度 / 传播速度）
2. ==分组丢失==：链路的队列缓冲区容量有限，当一个队列满后出现分组丢失，可能重传，可能不重传。
3. ==吞吐量==：数据在源端和目标端之间传输的速率。

----

### 7.协议层次和服务模型

1. 互联网是一个非常庞大的系统，为了实现这个庞大的系统，我们一般有两种方法：分模块 和 分层。

	> 互联网采取==分层==的方式，==只有相邻层之间才能相互调用==。

2. ==服务==：==下层实体向上层实体==提供通信的能力。

3. ==上层系统调用接口访问下层提供的服务==。

4. ==服务访问点（SAP）==：

	> ==一个下层实体可能会同时为多个上层实体提供服务，服务访问点可以区分上层的实体==，以便下层的实体为对应的上层实体提供服务（==比如 Socket 套接字就是服务访问点==）。

5. ==原语==：==下层给上层提供服务的方式（就是函数，传输层提供 Socket 服务的方式为 Socket API==）。

6. ==服务 vs 协议==：

	> ==服务是垂直的==，上层和下层实体间通信的方式；
	>
	> ==协议是水平的==，同一层双方通信的方式。
...
8. ==上层传下来的信息为 SDU==，我们要==加上本层的控制信息比如报文头部，形成 PDU==（协议数据单元）。

9. ==Internet 协议栈==：

	> - ==应用层==：==网络应用、应用程序之间的通信==。
	> - ==传输层==：==主机、进程之间的通信==。在网络层提供的端到端通信基础上，==细分为进程到进程==，将不可靠的通信变成可靠地通信。
	> - ==网络层==：==为数据报从源到目的选择路由，主机主机之间的通信，端到端通信，不可靠==。
	> - ==数据链路层==：==相邻网络节点（交换机）间的数据传输==。
	> - ==物理层==：==在物理介质上传播 bit 流==。

10. ==ISO / OSI 参考模型==：

	> - ==表示层==：允许应用解释传输的数据，e.g.==加密，压缩==，机器相关的表示转换。
	> - ==会话层==：维持 Session，数据交换的同步，检查点，恢复。

11. ==封装和解封装==：

	> <img src="ComputerNetworks.assets/image-20240721212929692.png" alt="image-20240721212929692" style="zoom:67%;" />
...
1. ==服务器 / 客户端模式（CS 模式==）
	...
	> ==缺点：可扩展性差==，比如现在有10万个用户要请求我的服务，我现有的服务器必须要扩容，更换内存 CPU，甚至更换新的服务器，而更大的请求数量也意味着需要更大的网络带宽，那我还要更换网络，但也==只能换离服务器比较近的网络，离用户近的网络还是换不了==，==CS 模式的性能当用户数达到某一个阈值时，会断崖式下降==，由此可见，CS 模式的可扩展性较差。
	>
	> ==另一方面，可靠性也比较差==，都依赖于服务器提供服务，服务器宕机了就无法提供服务。

2. ==对等体模式（P2P==）

	> P2P系统==请求服务的数量增加，提供服务的数量也在增加==，因此性能会随着节点的增加而==平稳地下降==，而不是断崖式下降，==可扩展性更好==，同时都能提供服务，==可靠性更好==。

3. ==CS 和 P2P 的混合体==：

	> ==用户登录时通知服务器，用户之间的通讯为 P2P 模式==，比如==你能接受对方的文件，也能给对方发文件==，我们常见的==即时通讯系统==都是采取 CS 和 P2P 的混合体模式。

4. ==问题一：对进程进行寻址（编址）==

	> - ==IP (Host) + TCP / UDP (传输层协议) + port==。

5. ==问题二：传输层如何给应用层提供服务==

	> - ==Socket== 是一个标示，用于==标示传输过程中双方的 IP 和端口==。
	>
	> ---
	>
	> - ==TCP 的 Socket== 为一个==四元组（源 ip 端口，目标 ip 端口==，==本质是代表源、目标主机间的会话关系==），一方面==便于管理==，另一方面==使得穿过层级的信息少，便于维持长时间的通信==。
	>
	> - ==TCP 中应用层至少要向传输层提供的信息==：
	>
	> 	1. SDU，即需要传输的报文。
	> 	2. Socket（包含源、目标 ip port）
	>
	> 	![image-20240722224634414](ComputerNetworks.assets/image-20240722224634414.png)
	>
	> ---
	>
	> - ==UDP 的 Socket== 是一个==二元组（源 ip 端口，只代表端系统，不代表会话关系）==。
	>
	> - ==UDP 中应用层至少要向传输层提供的信息==：
	>
	> 	1. SDU
	> 	2. Socket（包含源 ip port）
	> 	3. 目标 ip port
	>
	> 	![image-20240722230409761](ComputerNetworks.assets/image-20240722230409761.png)

6. ==问题三：应用层如何使用传输层提供的服务==

	> - ==TCP 的 Socket 只是建立连接时，操作系统放回的一个整数，用于表示上述的四元组，不能把 Socket 简单等价为四元组==；实际情况中，==应用层通过 Socket 这个整数得到源、目标 ip port，据此找到操作系统创建的 Socket，从而接收和发送消息；可以理解为操作系统维护了一张表，里面有 Socket 和四元组，因此可以通过四元组查询到 Socket==。
	> - ==UDP 操作系统可以根据目标 ip port 找到 Socket，然后接收和发送消息==。

7. ==套接字（Socket==）：进程向 Socket 发送报文或从 Socket 接收报文。

8. ==应用层协议==：规定不同端系统上的应用程序在应用层间如何通过报文，通过什么格式的报文进行通信。

9. 如何==描述传输层向应用层提供的服务==：==数据丢失率，吞吐，延迟，安全性==。

	> ![image-20240722231832971](ComputerNetworks.assets/image-20240722231832971.png)
...
10. ==TCP vs UDP==：
	...
	> 	3. ==流量控制==
	> 	4. ==面向连接==
	> 	5. 缺点：==延迟较高==，毕竟涉及到握手和挥手来维持可靠性嘛
	> 	6. 适用于==文件传输，邮件传输==
	> - ==UDP==：
	> 	1. 不可靠
	> 	2. ==面向报文==的协议
	> 	3. 不用连接，不会重传，==延迟低==，适用于==即时通讯==
	> - ![image-20240722233050532](ComputerNetworks.assets/image-20240722233050532.png)

11. 安全 TCP：通过==表示层的 SSL 实现==。
...
1. 服务器在 80 端口监听，创建==守护 Socket（Waiting Socket），专门用于等待建立请求==。在发现==用户连接后，额外创建 Socket 与用户保持连接==，在应用层通过 HTTP 报文进行通信。

2. > ==HTTP 协议是无状态的协议==，即==服务器不会维护任何和用户相关的信息==。设计为无状态是为了==减少报文中携带的数据量==，从而==降低传输时延==；并且==无状态的服务器可以支持更多客户端==。

3. ==非持久 HTTP==：==一个 TCP 连接只能传输一个对象==（如 HTML 网页、文件等）

	==持久 HTTP==：同一个 TCP 连接上可以传输==多个对象==。

4. ==响应时间模型 RTT==：

	> 一个小的==分组==从客户端到服务器，在回到客户端的时间（==一个来回的时间==）。==非持久 HTTP 传输至少用时 2RTT==，==1RTT 用于建立连接，1RTT 用于传输分组并回来等待响应==。

5. ==非流水线持久 HTTP==：必须收到==前一个对象的响应，才能发送下一个对象==。

	==流水线持久 HTTP==：无需等待上一个对象的响应，==发出第一个后可立马发第二个（类似异步==）

6. ==HTTP 请求报文==：

	> ==请求行、请求头、请求体==。==GET 请求没有请求体==，其数据在请求头中；==POST 请求有请求体，里面存储表单提交的数据==。

7. ==HTTP 响应报文==：

	> ==状态行（状态码）、首部行、数据（客户端请求的，由服务器返回的数据==）

8. ==Cookie==：

      > ==Cookie 是对 HTTP 协议无状态的一个补充==，主要用于==维持用户的状态==，比如==登录状态、购物车状态、视频喜爱状态==等。由服务器创建发给客户端，==存储在客户端的磁盘上==，在下一次客户访问同一域名下的网页时（==Cookie 跨域问题==），自动携带 Cookie，以便服务器获取同步信息，同步你的状态。

9. ==Web 缓存==：

	> Web 缓存类似于反向代理，缓存的原理类似于 Redis，就是先访问==浏览器本地的缓存==，没有就从服务器把返回结果存放到缓存服务器。（80% 的用户访问 20% 的请求，设置缓存可以==极大缓解带宽压力==）
...
1. FTP：==文件传输协议==。
1. FTP 是一个==有状态==的协议。用户必须主动给服务器提高验证信息，身份确认后，才能 upload 或者 download 一些文件。
1. 我们通过 FTP 客户端软件间接地和服务器打交道。
1. 服务器默认的 FTP 服务的端口是21号端口。
...
2. 常见的邮件代理软件如 outlook、fox mail。

3. ==SMTP 协议==，传输邮件的==过程如下==：

	> 1. sender 通过 SMTP 协议，将邮件从==代理软件==传给发送方的==邮件服务器==。
	> 2. ==发送方的邮件服务器==，通过 SMTP 协议，将邮件传给==接收方的邮件服务器==。
	> 3. ==receiver== 通过 ==POP3，IMAP，或者 HTTP== 协议，从接收方的邮件服务器将邮件拉取到代理软件。

4. ==SMTP 只能传输 Ascll 码==，即中文还有附件都传不了；但是为什么我们现在使用 SMTP 协议收发邮件可以发中文呢，打补丁。

5. ==SMTP 协议中，多个对象被封装到同一个邮件中==，比如里面有图片，有视频，有文件等；而 ==HTTP 响应报文中则只能放置一个对象==。

6. 前面说到了打补丁，那么具体怎么打的呢？通过某一套双方都遵守的协议，将多个不在 Ascll 码编码范围内的字符，转换成一个在 Ascll 范围内的字符，再通过 SMTP 协议发送。

6. ==POP3 协议是无状态的==；==IMAP 协议是有状态的==，因为提供远程文件夹目录的维护，所以服务器不仅要知道你是谁，还必须维护你的登录状态，从而实现远程目录的维护。
